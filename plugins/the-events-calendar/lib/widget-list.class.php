	...
	class TribeEventsListWidget extends WP_Widget {
      ...
      do_action( 'tribe_events_before_list_widget' );
			...
			if ( $posts ) {
				...
				echo '<section class="widget event"><ul><li><time datetime="2014-02-26">';
						the_time('D'); 
				echo	'<strong>';
						the_time('d');
				echo	'</strong>';
				...
				echo	'<span>';
						the_time('Y');
				echo	'</span></time><ul class="events">';
				...
				echo '<a href="' . $event_url . '" rel="bookmark" class="btn">';
				if ( empty( $category ) ) {
					_e( 'See all events', 'tribe-events-calendar' );
				} else {
					_e( 'See all events in Category', 'tribe-events-calendar' );
				}
				echo '</a></section>';
			}
			else {
